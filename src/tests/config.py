...
class Config:
    ...
    def __init__(self) -> None:
        ...
        with open("samconfig.toml", mode="r") as f:
            config = toml.load(f)
            self.stack_name = config['default']['global']['parameters']['stack_name']
            for entry in config['default']['deploy']['parameters']['parameter_overrides'].split(" "):
                key, value = entry.split("=")
                if key == 'PartnerIdParameter':
                    self.spmid = value.replace("\"", "")
                if key == 'StageParameter':
                    self.stage = value.replace("\"", "")
                if key == "WorkflowNotificationEmail":
    ...
    def api_gateway_url(self) -> str:
        if not self._api_gateway_url:
            ...
            self._api_gateway_url = [output['OutputValue']
                                     for output in outputs if output['OutputKey'] == 'ApiGatewayURL'][0]
